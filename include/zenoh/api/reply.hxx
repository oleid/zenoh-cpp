   ...
   public:
    ...
    const Sample& get_ok() const {
        if (!::z_reply_is_ok(interop::as_loaned_c_ptr(*this))) {
            THROW_ZEXCEPTION("Reply data sample was requested, but reply contains error", Z_EINVAL);
        }
    ...
    const ReplyError& get_err() const {
        if (::z_reply_is_ok(interop::as_loaned_c_ptr(*this))) {
            THROW_ZEXCEPTION("Reply error was requested, but reply contains data sample", Z_EINVAL);
        }
